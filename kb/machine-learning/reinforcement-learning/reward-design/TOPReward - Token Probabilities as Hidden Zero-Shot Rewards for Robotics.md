...
- 输入：视频前缀（帧序列）+ 语言指令；
- 输出：每个时刻的"任务完成似然"——既可作为跨轨迹的成功检测原始分数，也可经逐轨迹 min-max 归一化后作为轨迹内进度曲线。

**Token Probability 如何构造机器人奖励**：作者认为进度估计是价值（value）的良好代理；与其让 VLM 输出完成百分比（受数值生成与指令跟随偏差影响），不如直接查询模型对"任务是否已完成"的信念（belief），用肯定 token 的 log 概率作为进度 / 奖励信号。这是一种**概率化、非自回归**的进度估计。 [PAPER]

## 实验设定

- **ManiRewardBench**（作者提出的真实操作基准）：覆盖 **130 个唯一任务**（项目页详细拆分表列为 113 个任务 / 497 条 episode），跨越四个机器人平台（Franka Emika、单臂 / 双手 YAM、SO-100/101），带任务进度的时序标注；失败子集（23 个任务）用于成功检测评估； [PAPER]
- **Open X-Embodiment** 数据集（项目页报告为 39 个数据集、780 条 episode）； [PAPER]
- 对比基线 / 后端 VLM：GVL，以及 Molmo-2-8B、Qwen3-VL-8B、Gemini-2.5-Pro 等。 [PAPER]

## 主要结果（作者报告）

- **进度估计（Mean VOC，越高越好）**：在开源 VLM 上显著优于先前免训练方法——Open X-Embodiment 上 Qwen3-VL 达 0.857（GVL 仅 0.194，Molmo-2 为负）；ManiRewardBench 上 Qwen3-VL 达 0.947，优于 GVL；与训练过的奖励模型基线在进度估计指标上具竞争力，且**无需奖励模型训练**。 [PAPER]
...
- **真实部署（单臂 SO-100）**：用每任务仅 50 条带噪演示，结合 TOPReward 做优势加权回归（TOP-AWR），在 6 个任务上成功率 consistently 高于标准行为克隆（Behavior Cloning，BC）——例如 Place doll in box 0→7→10、Pick up cube 4→7→10、Put cube in cup 4→6→9；在最难任务上达到 10/10，而 BC 仅 7/10。 [PAPER]
